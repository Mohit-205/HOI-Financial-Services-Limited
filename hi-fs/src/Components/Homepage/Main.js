import left from './Images/homepage-banner-illustration left.svg';
import right from './Images/homepage-banner-illustration right.svg';
import car from './Images/car.svg';
import renew from './Images/renew.svg';
import health from './Images/health.svg';
import car_quote from './Images/car_quote.svg';
import {Link, useHistory} from 'react-router-dom';
import {Toaster} from 'react-hot-toast';
import {useFormik} from 'formik';
import { carnumberValidate } from '../Login/Validate';

function Main() {
  const navigate = useHistory();
  // Car number plate validation variable
  const formik = useFormik({
    initialValues : {
        carplate : ''
    },
    validate : carnumberValidate,
    validateOnBlur: false,
    validateOnChange: false,
    onSubmit : async values => {
      console.log(values); 
      navigate.push('/cars/useofcar');
    }
  })
  return (
        ...
        <div className="main_types">
          <div>
            <div className="main_car car1">
              <button className='carbtn'><img src={car} alt="car"></img></button>
            </div>
            <div className="main_car_text main_car_text1">Car & Taxi</div>
          </div>
              
          <div>
            <div className="main_car health1">
              <Link to="/Health"><button className='healthbtn'><img src={health} alt="health"></img></button></Link>
            </div>
            <div className="main_car_text main">Health</div>
          </div>

          <div>
            <div className="main_car renew1">
              <button className='renewbtn'><img src={renew} alt="renew"></img></button>
            </div>
            <div className="main_car_text">Policy Renewal</div>
          </div>
      ...
      <div className="home_main_mid_divs">
        <div className="custom-search">
          <form onSubmit={formik.handleSubmit}>
            <input {...formik.getFieldProps('carplate')}
              type="text"
