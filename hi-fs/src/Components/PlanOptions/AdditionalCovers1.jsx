...
export const AdditionalCovers1 = () => {
  var data;
  ...
  let totalPrice = smartSaverZeroDepreciationPlan;

  const AdditionalCoverSelected = (price) => {
    setAdded((props) => Number(props) + Number(price));
  };
  totalPrice = +smartSaverZeroDepreciationPlan + +added;
  const sendData = () => {
    // https://acko.herokuapp.com/user
    axios
      .post(`http://localhost:8000/user`, {
        selectedPlan: "Own Damage Plan",
        mobile: carDetails.mobile,
        premium: +smartSaverZeroDepreciationPlan + (smartSaverZeroDepreciationPlan * 20) / 80,
        paCover: +added,
        ncbDiscountAmount: (+smartSaverZeroDepreciationPlan * 20) / 80,
      })
      .then((res) => {
          ...
          <div className={styles.insideContRight}>
            ...
            <div>
              ...
              <div>
                ...
                <div>
                  <button 
                  onClick={event =>  window.location.href='https://buy.stripe.com/test_eVaaHp9Gpfiyfv28wx'}
                    // onClick={() => {
                    //   sendData();
